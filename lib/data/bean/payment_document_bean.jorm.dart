...
part of 'payment_document_bean.dart';

// **************************************************************************
// BeanGenerator
// **************************************************************************

abstract class _PaymentDocumentBean implements Bean<PaymentDocument> {
  final id = IntField('id');
  final collectionId = IntField('collection_id');
  final invoiceId = IntField('invoice_id');
  final amount = StrField('amount');
  final postFlag = StrField('post_flag');
  ...
  PaymentDocument fromMap(Map map) {
    PaymentDocument model = PaymentDocument();
    ...
    model.postFlag = adapter.parseValue(map['post_flag']);
  ...
  List<SetColumn> toSetColumns(PaymentDocument model,
    ...
    if (only == null && !onlyNonNull) {
      ...
      ret.add(postFlag.set(model.postFlag));
      ...
      if (only.contains(postFlag.name)) ret.add(postFlag.set(model.postFlag));
  ...
  Future<void> insertMany(List<PaymentDocument> models,
  ...
  Future<dynamic> upsert(PaymentDocument model,
      {bool cascade = false,
      Set<String> only,
      bool onlyNonNull = false,
  ...
  Future<void> upsertMany(List<PaymentDocument> models,
      {bool onlyNonNull = false,
      Set<String> only,
      isForeignKeyEnabled = false}) async {
  ...
  Future<int> update(PaymentDocument model,
      {bool cascade = false,
  ...
  Future<void> updateMany(List<PaymentDocument> models,
      {bool onlyNonNull = false, Set<String> only}) async {
    final List<List<SetColumn>> data = [];
  ...
  Future<PaymentDocument> find(int id,
  ...
  Future<int> removeMany(List<PaymentDocument> models) async {
