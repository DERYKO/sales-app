...
part of 'must_have_sku_bean.dart';

// **************************************************************************
// BeanGenerator
// **************************************************************************

abstract class _MustHaveSkuBean implements Bean<MustHaveSku> {
  final id = IntField('id');
  final customercategory = IntField('customercategory');
  final productId = IntField('product_id');
  final status = StrField('status');
  final addedby = IntField('addedby');
  final createdAt = DateTimeField('created_at');
  final updatedAt = DateTimeField('updated_at');
  Map<String, Field> _fields;
  Map<String, Field> get fields => _fields ??= {
        id.name: id,
        customercategory.name: customercategory,
  ...
  MustHaveSku fromMap(Map map) {
    MustHaveSku model = MustHaveSku();
    model.id = adapter.parseValue(map['id']);
    model.customercategory = adapter.parseValue(map['customercategory']);
    model.productId = adapter.parseValue(map['product_id']);
    model.status = adapter.parseValue(map['status']);
    model.addedby = adapter.parseValue(map['addedby']);
  ...
  List<SetColumn> toSetColumns(MustHaveSku model,
      {bool update = false, Set<String> only, bool onlyNonNull = false}) {
    List<SetColumn> ret = [];

    if (only == null && !onlyNonNull) {
      ret.add(id.set(model.id));
      ret.add(customercategory.set(model.customercategory));
      ...
      if (only.contains(customercategory.name))
        ret.add(customercategory.set(model.customercategory));
  ...
  Future<void> createTable({bool ifNotExists = false}) async {
    ...
    st.addInt(customercategory.name, isNullable: true);
  ...
  Future<dynamic> insert(MustHaveSku model,
      {bool cascade = false,
  ...
  Future<void> insertMany(List<MustHaveSku> models,
  ...
  Future<dynamic> upsert(MustHaveSku model,
      {bool cascade = false,
      Set<String> only,
      bool onlyNonNull = false,
  ...
  Future<void> upsertMany(List<MustHaveSku> models,
      {bool onlyNonNull = false,
      Set<String> only,
      isForeignKeyEnabled = false}) async {
  ...
  Future<int> update(MustHaveSku model,
      {bool cascade = false,
  ...
  Future<void> updateMany(List<MustHaveSku> models,
      {bool onlyNonNull = false, Set<String> only}) async {
    final List<List<SetColumn>> data = [];
  ...
  Future<MustHaveSku> find(int id,
  ...
  Future<int> removeMany(List<MustHaveSku> models) async {
