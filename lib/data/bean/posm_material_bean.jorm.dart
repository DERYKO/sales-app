...
part of 'posm_material_bean.dart';

// **************************************************************************
// BeanGenerator
// **************************************************************************

abstract class _PosmMaterialBean implements Bean<PosmMaterial> {
  final id = IntField('id');
  final itemname = StrField('itemname');
  final itemtype = StrField('itemtype');
  ...
  PosmMaterial fromMap(Map map) {
    PosmMaterial model = PosmMaterial();
    model.id = adapter.parseValue(map['id']);
    model.itemname = adapter.parseValue(map['itemname']);
    model.itemtype = adapter.parseValue(map['itemtype']);
  ...
  List<SetColumn> toSetColumns(PosmMaterial model,
    ...
    if (only == null && !onlyNonNull) {
      ...
      ret.add(itemtype.set(model.itemtype));
      ...
      if (only.contains(itemtype.name)) ret.add(itemtype.set(model.itemtype));
  ...
  Future<void> createTable({bool ifNotExists = false}) async {
    ...
    st.addStr(itemtype.name, isNullable: true);
  ...
  Future<dynamic> insert(PosmMaterial model,
      {bool cascade = false,
  ...
  Future<void> insertMany(List<PosmMaterial> models,
  ...
  Future<dynamic> upsert(PosmMaterial model,
      {bool cascade = false,
      Set<String> only,
      bool onlyNonNull = false,
  ...
  Future<void> upsertMany(List<PosmMaterial> models,
      {bool onlyNonNull = false,
      Set<String> only,
      isForeignKeyEnabled = false}) async {
  ...
  Future<int> update(PosmMaterial model,
      {bool cascade = false,
  ...
  Future<void> updateMany(List<PosmMaterial> models,
      {bool onlyNonNull = false, Set<String> only}) async {
    final List<List<SetColumn>> data = [];
  ...
  Future<PosmMaterial> find(int id,
  ...
  Future<int> removeMany(List<PosmMaterial> models) async {
