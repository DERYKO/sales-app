...
class ProductAvailabilityBloc extends Bloc {
  var pickedDates = [
    DateTime.now(),
    DateTime.now(),
  ];
    ...
    if (connectionManager.isConnected) {
      if (await syncManager.shouldSync) {
        ...
        if (!syncManager.syncing) {
          productAvailabilityManager.loadAvailability(pickedDates);
        }
      }
    } else {
      alert(
  ...
  void initState() {
    super.initState();
    print(
        "AvItems ${productAvailabilityManager.getAvailabilityDetailsById(66).length}");
  }
}
